...
- 러닝 기록 업로드 · 저장 기능 구현  
  ...
  - 시간은 내부에서 초 단위(`time`)로 변환하여 저장  
  - `Run` 스키마 설계: `distance`, `time`, `createdAt`
- 러닝 기록 조회 UI 구현  
  - 로그인한 사용자의 기록만 조회  
  - 리스트 형태로 최근 기록 표시  
...
### **4주차 (12.01 ~ 12.07)**
> 🔥 **주요 목표: 사용자 간 대결 + 오픈 매치**
- 유저 대결 생성/수락 API (`/api/competitions/user`, `/api/competitions/:id/accept`)
- 오픈 대결 목록 조회 UI 및 참여 버튼
- 대결 삭제/정리 UX 추가
- 소켓 이벤트로 대결/리더보드 실시간 업데이트

### **5주차 (12.08 ~ 12.14)**
> 🔥 **주요 목표: 리더보드/주간 통계 고도화**
- 주간 리더보드 집계 API (`/api/leaderboard/weekly`)
- 주간 합산/평균 페이스/횟수 계산 및 내 순위 표시
- 차트(거리·시간 바차트, 페이스 라인차트)로 최근 10회 시각화
- 대시보드 정보 구조/스타일 다듬기

### **6주차 (12.15 ~ 12.21)**
> 🔥 **주요 목표: AI 코칭(규칙 기반) 연동**
- AI 코칭 API (`/api/coach/insights`): 최근 30회 기록으로 페이스/주간 볼륨/최장 거리 분석 후 요약·추천 반환
- 대시보드 AI Coach 카드: 요약, 핵심 지표(최근/최고 페이스, 주간 거리, 최장 거리), 인사이트/추천 리스트 표시
- 기록 추가/삭제 시 코칭 자동 새로고침 및 수동 재분석 버튼 추가
- GPT 등 외부 모델 연동 전 단계: 서버 내 규칙 기반 요약(추후 OpenAI 키 연결 시 프록시 가능)
